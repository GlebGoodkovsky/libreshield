...
- **`webRequest`** and **`webRequestBlocking`:** Allow the extension to observe and block network requests. Both are required in Firefox to intercept and redirect requests to blocked domains.
- **`alarms`:** Allows the extension to schedule periodic tasks, used for cleaning up expired temporary unblocks and resetting daily stats.
...
- Storing your password as a `PBKDF2` hash.
